...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("📝 Create profile request:", { ...body, profileImage: body.profileImage ? "***" : "" })
    ...
    try {
      ...
      if (!response.ok) {
        console.error("❌ Real API create profile error:", data)
    ...
    console.error("💥 Create profile proxy error:", error.message)
